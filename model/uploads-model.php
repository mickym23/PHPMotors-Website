...
function checkExistingImage($imgName){
  ...
  function getThumbnailsByInvId($invId) {
    $db = phpmotorsConnect();
    $sql = 'SELECT i.invId, i.invMake, i.invModel, img.imgPath AS "invThumbnail" FROM inventory i LEFT JOIN images img ON i.invId = img.invId AND img.imgPath LIKE "%-tn%" WHERE i.invId = :invId';
    $stmt = $db->prepare($sql);
