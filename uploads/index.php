...
switch ($action) {
 ...
 if($imageCheck){
  $message = '<p class="message2">An image by that name already exists.</p>';
 } elseif (empty($invId) || empty($imgName)) {
  $message = '<p class="message2">You must select a vehicle and image file for the vehicle.</p>';
 } else {
  ...
  if ($result) {
   ...
   $message = '<p class="message2">Sorry, the upload failed.</p>';
  }
 }
...
if ($remove) {
 ...
 $message = "<p class='message2'>$filename was NOT deleted.</p>";
}
...
if (count($imageArray)) {
 ...
 $imageDisplay = '<p class="message2">Sorry, no images could be found.</p>';
}
